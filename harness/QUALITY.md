...
현재 전체 파이프라인은 개발자가 진행 상태를 바로 파악할 수 있도록 다음 3단계 로그를 출력한다.
...
Enrichment는 현재 `make enrich` 또는 `make trace-enrich`로 독립 실행한다. Writer 연결 이후 전체 파이프라인 로그를 Collector, Preprocessor, Enrichment, Writer 4단계로 확장한다.
