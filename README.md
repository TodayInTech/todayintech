...
사이트 파비콘, 내비게이션 로고, README는 light/dark SVG를 자동 전환하며, GitHub 조직 아이콘처럼 PNG 업로드가 필요한 곳에는 [조직 아이콘 PNG](static/img/brand/todayintech.organization.png)를 사용합니다.
...
8. 생성된 `docs/`와 `data/briefed_articles.json`을 main에 커밋해 누적 아카이브 상태 보존
9. `make build`로 Docusaurus 빌드
10. GitHub Pages 배포
...
- `briefed_articles` 상태는 Writer 생성 성공 후 갱신되며, GitHub Actions가 main에 커밋한 뒤 이후 전처리 필터로 사용합니다.
