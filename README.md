...
현재 저장소는 MVP 기본 구조 단계입니다. 서비스별 정보 수집, raw snapshot 저장, 운영 trace, Docusaurus 빌드/배포 흐름이 잡혀 있으며, 중복 없는 글 단위 브리핑과 LLM 기반 선별/요약은 이후 구현 예정입니다.
...
전체 파이프라인을 실행하여 수집 결과와 Markdown을 함께 생성합니다. 현재 구현은 MVP scaffold이며, 다음 단계에서 preprocessor와 article archive generator로 전환합니다.
...
- 날짜별 일간 브리핑 구조는 폐기 예정이며, article archive 구조로 전환합니다.
- LLM 기반 요약/중요도 평가는 아직 실제 구현 전입니다.
- 현재 요약, 분류, 점수는 MVP 휴리스틱입니다.
- `briefed_articles` 기반 재처리 방지는 아직 구현 전입니다.
