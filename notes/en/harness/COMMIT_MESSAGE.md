...
## Automatically Generated Archive Commits

GitHub Actions uses the following format to distinguish generated briefing archive commits from regular development commits.

```text
Update Today in Tech — YYYY-MM-DD

Archive changes generated by the Daily Briefing workflow.
...
## Added Sources
- [Added file path](GitHub file URL)
```

- The production commit title is written in Korean as `todayintech 갱신 YYYY-MM-DD`.
- A manually supplied `target_date` is used in the title when present; otherwise, the workflow run date in Korea Standard Time is used.
- The source section is generated only from newly added files in the staged diff.
- Each source is rendered as a link that opens the file on the target branch in GitHub.
- The section displays `None` when no sources were added.
- Run information includes the target date and a link to the corresponding GitHub Actions run.
- Commits created by the workflow use `choi-hyk` as the author.
